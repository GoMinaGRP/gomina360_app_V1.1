...
// ── 2. enable + materialization for TODAY ──────────────────────────────────
{
  if (!FRESH) {
    ...
    // Self-healing (order-robustness): a crashed earlier suite can leave
    // system stage rows deactivated. The enable action is the documented
    // Owner path that reactivates them — run it so this suite always starts
    // from the state it asserts (stage plan ON), then generate today.
    await POST({ entity: "STAGE_PLAN", data: { businessId: POULTRY_ID, action: "enable", cutoffHour: 18 } });
    await POST({ entity: "GENERATE", data: { businessId: POULTRY_ID, checklistDate: today } });
  }
  ...
  // Order-robustness (self-healing): earlier suites or demo seeders may have
  // materialized today's entries for flocks that are CLOSED/SOLD by now
  // (entries persist — GENERATE never rewrites history). Purge those stale
  // rows so this section measures exactly what a fresh GENERATE produces.
  await q("delete from checklist_entries where checklist_date=$2 and business_id=$1 and flock_id in (select id from poultry_flocks where business_id=$1 and status <> 'ACTIVE')", [POULTRY_ID, today]);
  ...
  const flockIdOf = async (batch) => Number((await q1("select id from poultry_flocks where business_id=$1 and batch_number=$2", [POULTRY_ID, batch]))?.id);

  const b01 = byFlock.get(await flockIdOf("BENCH-DEMO-B01")) || []; // broiler, stocked ~30d ago by seed-benchmark-demo
  ...
  const l01 = byFlock.get(await flockIdOf("BATCH-2026-L01")) || []; // layer, arrival 2025-09-15 → wk ~53
  ...
  const l03 = byFlock.get(await flockIdOf("BATCH-2026-L03")) || []; // layer, arrival 2026-02-20 → wk ~30
  ...
  // CLOSED/SOLD flocks are resolved from live data — flock ids drift with the
  // DB's serial history (test suites consume ids), never hardcode them.
  const closedFlocks = new Set((await q("select id from poultry_flocks where business_id=$1 and status <> 'ACTIVE'", [POULTRY_ID])).map((f) => Number(f.id)));
...
{
  const rows = await entriesOf(today);
  // Pick the MOST RECENTLY materialized critical flock task: the /api/audit
  // record view builds checklist records from the latest 240 entries, so a
  // low-id demo entry could fall outside that window while §3's test-flock
  // entries are live.
  const crit = rows
    .filter((e) => e.priority === "CRITICAL" && e.flock_id != null && !e.is_completed)
  ...
  if (crit) {
    ...
    // The audit write is deliberately fire-and-forget in the API — retry briefly.
    let trail = 0;
    for (let i = 0; i < 5 && trail === 0; i++) {
      await new Promise((r2) => setTimeout(r2, 200));
      trail = await count("select count(*) c from audit_trail where action='CHECKLIST_CRITICAL_DONE' and record_id=$1", [crit.id]);
    }
    ...
    for (let i = 0; i < 3 && !rec; i++) {
      ...
      rec = (aud.d?.records || []).find((x) => x.key === `CHECKLIST:checklist_entries:${crit.id}`);
      if (!rec) await new Promise((r) => setTimeout(r, 400)); // async audit write settle
    }
...
// ── 7. overdue sweep (PER-FLOCK: one notification per flock+date) ─────────
{
  // V2: recordRef = checklist-overdue:{biz}:{flockId|farm}:{date}; recordId
  // links the notification to the right flock.
  const refPat = `checklist-overdue:${POULTRY_ID}:%:${today}`;
  const legacyRef = `checklist-overdue:${POULTRY_ID}:${today}`;
  await q("delete from notifications where record_ref=$1", [legacyRef]);
  await q("delete from notifications where record_ref like $1", [refPat]); // fresh
  ...
  const n = await q1("select count(*) c, max(title) t from notifications where type='CHECKLIST_OVERDUE' and record_ref like $1", [refPat]);
  ok("overdue sweep → CHECKLIST_OVERDUE notification(s)", Number(n.c) >= 1, `“${n.t}”`);
  const withFlock = await q1(
    "select count(*) c from notifications n where n.type='CHECKLIST_OVERDUE' and n.record_ref like $1 and n.record_id is not null",
    [refPat],
  );
  ok("overdue notifications are flock-linked (record_id set)", Number(withFlock.c) >= 1, `${withFlock.c} flock-linked`);
  const r2 = await POST({ entity: "SWEEP", data: { businessId: POULTRY_ID, cutoffHour: 0 } });
  const n2 = await q1("select count(*) c from notifications where type='CHECKLIST_OVERDUE' and record_ref like $1", [refPat]);
  ok("re-sweep deduped (one per flock+date)", Number(n2.c) === Number(n.c), `${n.c} → ${n2.c}`);
...
// ── 9. non-poultry business unaffected ─────────────────────────────────────
{
  ...
  ok("BLOCK-01 generation stays business-level", rows.length > 0 && rows.every((e) => e.flock_id == null && e.stage_key == null), `${rows.length} entries`);
