...
// H3: self-clean by default — purge this run's (and any past) MW-* fixtures so
// the demo tenant stays clean; KEEP=1 retains them for forensics.
async function fixtureCleanup() {
  if (process.env.KEEP === "1") { console.log(`KEEP=1 — fixture tag retained: ${tag}`); return; }
  try {
    ...
    await pg.end();
    console.log(`self-clean: purged businesses=${out.businesses} organizations=${out.organizations} users=${out.users}`);
