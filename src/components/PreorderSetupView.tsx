  ...
  const saveMethod = async () => {
    ...
    try {
      await api({
        ...
        key: methodDraft.key,
        label: methodDraft.label,
        icon: methodDraft.icon || "truck",
        defaultLeadMinDays: Number(methodDraft.leadMin) || 7,
        defaultLeadMaxDays: Number(methodDraft.leadMax) || 14,
        businessScoped: Boolean(methodDraft.businessScoped),
      });
      setShowMethod(false);
      setMethodDraft({});
      setFlash("Fulfilment method added.");
      await loadCatalogue();
    } catch (e: any) {
      setError(e.message);
    } finally {
  ...
  const saveOption = async () => {
    ...
    try {
      const payload: any = {
        ...
        priceGhs: Number(optionDraft.priceGhs),
        leadMinDays: Number(optionDraft.leadMinDays) || 7,
        leadMaxDays: Number(optionDraft.leadMaxDays) || 14,
        depositType: optionDraft.depositType || "NONE",
        depositValue: Number(optionDraft.depositValue) || 0,
    ...
    <div className="space-y-4" data-testid="po-root">
      <div className="flex flex-wrap items-center justify-between gap-2 bg-slate-800/90 border border-slate-700/80 p-4 rounded-xl">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setMethodDraft({}); setShowMethod(true); }} className="px-3 py-2 rounded-lg bg-slate-700 hover:bg-slate-600 text-white text-xs font-bold flex items-center gap-1.5" data-testid="po-new-method">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" data-testid="po-method-modal">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-5 w-full max-w-md space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-extrabold text-white">New fulfilment method</h4>
            ...
            <div className="space-y-3">
              <div><label className="block text-[11px] font-semibold text-slate-400 mb-1">Key (short word)</label>
                <input value={methodDraft.key || ""} onChange={(e) => setMethodDraft({ ...methodDraft, key: e.target.value.toUpperCase() })} className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white text-sm" placeholder="BOAT" data-testid="po-m-key" /></div>
            ...
            <div className="flex gap-2 justify-end">
              <button onClick={() => setShowMethod(false)} className="px-3 py-2 rounded-lg bg-slate-700 text-white text-xs font-bold">Cancel</button>
              <button onClick={saveMethod} disabled={busy} className="px-3 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold disabled:opacity-50" data-testid="po-m-save">Create</button>
            </div>
          </div>
        </div>
      )}
