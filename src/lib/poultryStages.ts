...
// ─── Day / Week display (field convention: placement day = Day 1) ─────────

/** Display day of age — the arrival day is "Day 1". */
export const displayDayOf = (ageDays: number) => Math.max(0, Math.floor(ageDays)) + 1;
/** Display week of age — the first 7 days are "Week 1". */
export const displayWeekOf = (ageDays: number) => Math.floor(Math.max(0, ageDays) / 7) + 1;

// ─── Effective per-flock plan (pure, client-safe) ───────────────────────────

/**
 * The plan a flock actually runs:
 *  - Flock's OWN rows (any origin, active) REPLACE the system stage plan for
 *    that flock — one flock's customization never touches another flock or
 *    the system plan itself.
 *  - Farm-wide scoped custom items (birdType/stage scoped, flockId null)
 *    still apply on top; taskKey collisions resolve in the flock row's favor.
 *  - No own rows → the recommended system stage plan + farm-wide custom items.
 * Mirrors the generator's effectivePlanForFlock exactly (single source of
 * truth — the generator delegates here).
 */
export function effectivePlanItemsForFlock(
  flock: { id: number; birdType: string },
  allTemplates: any[],
): any[] {
  const flockRows = allTemplates.filter(
    (t: any) => t.flockId != null && Number(t.flockId) === Number(flock.id) && t.isActive !== false,
  );
  const customScoped = allTemplates.filter(
    (t: any) =>
      t.origin !== "STAGE_PLAN" &&
      t.flockId == null &&
      t.isActive !== false &&
      (t.birdType != null || t.stageKeys != null),
  );
  if (!flockRows.length) {
    const stage = allTemplates.filter(
      (t: any) => t.origin === "STAGE_PLAN" && t.flockId == null && t.isActive !== false,
    );
    return [...stage, ...customScoped.filter((c: any) => !stage.some((t: any) => t.taskKey === c.taskKey))];
  }
  return [...flockRows, ...customScoped.filter((c: any) => !flockRows.some((r: any) => r.taskKey === c.taskKey))];
}

// ─── Lifecycle schedule projection ─────────────────────────────────────────
...
export interface PlanTaskLike {
  ...
  houseScoped?: boolean | null;
}

export interface ScheduleSlot {
  /** Broilers: display day (1-based). Layers: display week (1-based). */
  index: number;
  ageDays: number;
  stageKey: string;
  stageLabel: string;
...
/**
 * Project a flock's full lifecycle checklist schedule — the PLAN, not the
 * materialized history: broilers day-by-day from placement to closeout,
 * layers week-by-week from rearing to spent. Pure and deterministic, shared
 * by the UI timeline (past/future shading) and previews at flock start.
 *
 * Cadence rules mirror the generator's due windows, anchored at placement:
 * DAILY every slot; WEEKLY every 7 days (layers: every week); MONTHLY every
 * 28 days (layers: every 4th week); STAGE_ONCE on the slot where its stage
 * window opens.
 */
export function buildLifecycleSchedule(
  ...
  items: PlanTaskLike[],
  marketAgeDays?: number | null,
): LifecycleSchedule | null {
  const bt = String(birdType || "").toUpperCase();
  if (bt !== "BROILERS" && bt !== "LAYERS") return null;
  const m =
    bt === "BROILERS"
      ? (Number(marketAgeDays) > 0 ? Number(marketAgeDays) : DEFAULT_BROILER_MARKET_AGE_DAYS)
      : null;

  const applies = (t: PlanTaskLike, stageKey: string) => {
    if (t.birdType != null && String(t.birdType).toUpperCase() !== bt) return false;
    const sk = t.stageKeys;
    if (sk == null) return true;
    if (!Array.isArray(sk) || sk.length === 0) return false;
    return sk.includes(stageKey);
  };
  const freqOf = (t: PlanTaskLike) => String(t.frequency || "DAILY").toUpperCase();

  // last slot: broilers → market + 14 (closeout window); layers → week 86
  const totalDays = bt === "BROILERS" ? (m as number) + 14 : 86 * 7;
  const step = bt === "BROILERS" ? 1 : 7;
  const slots: ScheduleSlot[] = [];
  let lastStageKey: string | null = null;

  for (let d = 0; d < totalDays; d += step) {
    const flockLike = { birdType: bt, arrivalDate: "2000-01-01" };
    const date = new Date(Date.UTC(2000, 0, 1 + d));
    const iso = date.toISOString().slice(0, 10);
    const stage = stageOfFlock(flockLike, iso, m != null ? { curves: { _meta: { marketAgeDays: m } } } : null);
    if (!stage) continue;
    const isStageStart = stage.stageKey !== lastStageKey;
    lastStageKey = stage.stageKey;

    const tasks = items.filter((t) => {
      if (!applies(t, stage.stageKey)) return false;
      const f = freqOf(t);
      if (f === "DAILY") return true;
      if (f === "STAGE_ONCE") return isStageStart;
      if (bt === "BROILERS") {
        if (f === "WEEKLY") return d > 0 && d % 7 === 0;
        if (f === "MONTHLY") return d > 0 && d % 28 === 0;
      } else {
        if (f === "WEEKLY") return true; // one slot per week already
        if (f === "MONTHLY") return d > 0 && d % 28 === 0;
      }
      return true;
    });

    if (tasks.length > 0 || isStageStart) {
      slots.push({
        index: bt === "BROILERS" ? displayDayOf(d) : displayWeekOf(d),
        ageDays: d,
  ...
  return { birdType: bt as any, unit: bt === "BROILERS" ? "Day" : "Week", marketAgeDays: m ?? null, slots };
}
