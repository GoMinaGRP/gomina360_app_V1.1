...
export async function GET(
  ...
  try {
    ...
    // Operations logs ride the unit's section catalog: an advisor with no
    // permitted section of this unit's farm module gets nothing (403),
    // mirroring the /api/init bucket stripping.
    if (isFarmAdvisor(session.user)) {
      const moduleKey = farmModuleOfBusiness(biz);
      if (moduleKey) {
        const secs = await advisorSectionsForBusiness(session.user, Number(biz.id));
        if (!anySectionAllowed(secs, moduleKey)) {
          return FORBIDDEN("The farm owner has not given you access to this unit's operations.");
        }
      }
    }
